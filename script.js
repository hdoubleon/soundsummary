...
function showResult(tabId, type) {
    ...
    switch (type) {
        case 'summary':
            ...
            // 백엔드에서 받은 실제 요약본 사용 (쓰레기 값 생성하지 않음)
            const summaryContent = summary?.content || '요약본이 없습니다.';
            console.log('📄 요약본 탭 표시:', summaryContent.substring(0, 100) + '...');
            output.innerHTML = `
...
// Job 완료 폴링 함수
async function pollJobCompletion(jobId, maxAttempts = 1200, intervalMs = 3000) {
    ...
    throw new Error('작업 완료 확인 시간 초과 (1시간)');
}
...
/**
 * 백엔드에서 모든 요약 Job 목록을 가져옵니다.
 */
async function fetchAllJobsFromBackend() {
    try {
        const response = await fetch('http://localhost:8000/summary-jobs');
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const jobs = await response.json();
        console.log('✅ 백엔드에서 가져온 Jobs:', jobs);
        return jobs;
    } catch (error) {
...
/**
 * 특정 Job ID를 백엔드에서 삭제합니다.
 */
async function deleteJobFromBackend(jobId) {
    try {
        const response = await fetch(`http://localhost:8000/summary-jobs/${jobId}`, {
            method: 'DELETE'
        });
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
        ...
        console.log(`✅ Job ${jobId} 삭제 성공:`, result);
        return true;
    } catch (error) {
        console.error(`❌ Job ${jobId} 삭제 실패:`, error);
        return false;
    }
}

/**
 * 실패한 Job들을 백엔드에서 삭제합니다.
 */
async function cleanupFailedJobs() {
    console.log('🧹 실패한 Job들을 정리합니다...');
    const jobs = await fetchAllJobsFromBackend();
    
    const failedJobs = jobs.filter(job => 
        job.status === 'FAILED' || 
        job.status === 'PENDING' || 
        !job.source_materials || 
        job.source_materials.length === 0
    );
    
    console.log(`🔍 발견된 실패/불완전 Jobs: ${failedJobs.length}개`);
    
    for (const job of failedJobs) {
        console.log(`🗑️  Job ${job.id} (${job.status}) 삭제 중...`);
        await deleteJobFromBackend(job.id);
    }
...
/**
 * 백엔드에서 완료된 Job들을 가져와 프론트엔드 히스토리에 동기화합니다.
 */
async function syncJobsFromBackend() {
    console.log('🔄 백엔드와 동기화 중...');
    const jobs = await fetchAllJobsFromBackend();
    
    const completedJobs = jobs.filter(job => job.status === 'COMPLETED');
    console.log(`✅ 완료된 Jobs: ${completedJobs.length}개`);
    
    // sessionHistory 초기화하고 백엔드 데이터로 채우기
    sessionHistory = [];
    
    completedJobs.forEach(job => {
        if (job.source_materials && job.source_materials.length > 0) {
            job.source_materials.forEach(material => {
                if (material.status === 'COMPLETED') {
                    ...
                    if (material.speaker_attributed_segments) {
                        speakerSegments = material.speaker_attributed_segments.map(seg => ({
                            speaker: seg.speaker_label || 'Unknown',
                            start: parseFloat(seg.start_time_seconds) || 0,
                            end: parseFloat(seg.end_time_seconds) || 0,
                            text: seg.text
                        }));
                    }
                    ...
                    const summary = {
                        id: material.id,
                        jobId: job.id,
                        title: material.original_filename || `Job ${job.id}`,
                        type: 'file',
                        timestamp: new Date(job.created_at).toLocaleString('ko-KR'),
                        content: material.individual_summary || '요약 내용 없음',
                        speakerSegments: speakerSegments,
                        transcription: material.output_artifacts || {},
                        workspace: job.subject?.workspace?.name || null,
                        subject: job.subject?.name || null,
                        fileName: material.original_filename || 'unknown.audio',
                        audioUrl: null  // 로컬 파일은 URL 없음
                    };
    ...
    console.log('✅ 동기화 완료! sessionHistory:', sessionHistory);
    return sessionHistory.length;
}

/**
 * 전체 정리 및 동기화 (쓰레기 파일 삭제 + 요약 불러오기)
 */
async function cleanupAndSync() {
    console.log('🚀 전체 정리 및 동기화 시작...');
    
    // 1단계: 실패한 Job 삭제
    const deletedCount = await cleanupFailedJobs();
    console.log(`✅ ${deletedCount}개의 실패한 Job 삭제 완료`);
    
    // 2단계: 완료된 Job 동기화
    const syncedCount = await syncJobsFromBackend();
    console.log(`✅ ${syncedCount}개의 요약 불러오기 완료`);
    
    console.log('🎉 모든 작업 완료!');
    alert(`정리 완료!\n- 삭제: ${deletedCount}개\n- 불러오기: ${syncedCount}개`);
}

// 전역 스코프에 노출 (브라우저 콘솔에서 사용 가능)
window.fetchAllJobsFromBackend = fetchAllJobsFromBackend;
window.deleteJobFromBackend = deleteJobFromBackend;
